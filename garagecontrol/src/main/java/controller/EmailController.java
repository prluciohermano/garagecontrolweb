...
public class EmailController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (action.equals("/usuEmail") ) {
			System.out.println("E-mail de Recuperação");
			Locale.setDefault(new Locale("pt", "BR"));
			...
			if (!nomeEmail.equals("") && !nomeEmail.equals(null)) {
				...
				try {
					request.setAttribute("msg2", "Enviando recuperação de senha!");
					response.setCharacterEncoding("UTf-8");
					email.setFrom(meuEmail);
					email.setSubject("Senha do seu login");
					email.setMsg("Olá, " + nomeEmail + "\n\nSua senha é: " + "Teste de Senha" + "\n\n" + "Mensagem: Essa é sua senha cadastrada!");
